...
delimiter $
create trigger QuantidadeEstoque before update on produto for each row
	begin 
		declare qtd int;
        set qtd = produto.qtd = produto.qtd - new.qtd 
        where produto.id = new.id;
			
		if(qtd <= 0 ) then
			signal SQLSTATE '45000' SET MESSAGE_TEXT = "Estoque não é suficiente";
